...
def build_pipeline() -> StateGraph:
    ...
    # Agent 1 and Agent 2 run in parallel from START
    graph.add_edge(START, "geo_scoring")
    graph.add_edge(START, "web_intel")

    # Both feed into synthesis
    graph.add_edge("geo_scoring", "synthesis")
    graph.add_edge("web_intel", "synthesis")

    # Synthesis feeds report gen and KB update
